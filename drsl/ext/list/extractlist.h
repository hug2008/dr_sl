...
*/
#ifndef __DRSL_LIST_EXTRACTLIST_H_
...
void extractlist(T *str, std::vector<reference_string<T> > &list, const LIST_OPTIONS<T> *options, size_t strLength = -1)
{
    ...
    if (options != NULL)
    {
        drsl::tokens::TOKEN_OPTIONS<T> token_options;
        token_options.quotes = options->quotes;
        token_options.escapeCharacter = options->escapeCharacter;

        drsl::tokens::extracttokens(str, tokens, NULL, &token_options, strLength);
    }
    else
    {
        drsl::tokens::extracttokens(str, tokens, NULL, strLength);
    }
...
#endif // __DRSL_LIST_EXTRACTLIST_H_
